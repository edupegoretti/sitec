...
import { interestsQuery } from '@/sanity/lib/queries'
...
export default async function RecursosInteressesPage() {
  const interests = await sanityFetch<Interest[]>({ query: interestsQuery, tags: ['interest'] })
      ...
      <section className="py-16 sm:py-24 bg-linear-to-b from-gray-50 to-white">
        <Container>
          <div className="max-w-4xl mx-auto text-center">
            ...
            <Reveal delay={0.1}>
              <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-6">Interesses</h1>
            </Reveal>
            <Reveal delay={0.2}>
              <p className="text-xl text-gray-600 mb-10 max-w-3xl mx-auto">
                Interesses são dores e objetivos reais — perfeitos para buscar respostas rápidas e práticas.
              </p>
            </Reveal>
            <Reveal delay={0.3}>
              <Link
          ...
          <div className="max-w-6xl mx-auto">
            {interests.length === 0 ? (
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-8 text-center">
                <p className="text-lg font-bold text-gray-900">Nenhum interesse publicado</p>
                <p className="text-gray-600 mt-2">Crie interesses no Studio para habilitar a navegação.</p>
              </div>
            ) : (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {interests.map((interest) => (
                  <Reveal key={interest._id}>
                    <Link
                    ...
                    >
                      ...
                      {interest.description ? (
                        <p className="mt-2 text-sm text-gray-600 leading-relaxed">{interest.description}</p>
                      ) : (
